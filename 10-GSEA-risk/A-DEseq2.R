...
## 1.5 定义变量----
treat_name = "HighRisk"    # 疾病组命名
con_name = "LowRisk"  # 对照组命名

# 2 读取数据----
## 排序样本，高风险在前
TCGA_group <- data.table::fread("input/7-RiskScore_LASSO.csv", data.table = F,header = T) %>% 
  select("sampleid","risklevel") %>%
  dplyr::arrange(risklevel) ## 排序样本，高风险在前

colnames(TCGA_group) <- c("RNAseq样本编号","Group")
head(TCGA_group)
tail(TCGA_group)

## 矩阵样本与TCGA_group样本顺序一致
TCGA_mat <- data.table::fread("input/Disease_Matrix_Counts.csv", data.table = F) %>% 
...
data_label <- dat_volcanoplot[rownames(dat_volcanoplot) %in% DEGs,]
...
p_volcano
ggsave(file = 'output/3-vocano.pdf', p_volcano, width = 10, height = 7)
...
TCGA_group <- data.table::fread("input/7-RiskScore_LASSO.csv", data.table = F,header = T) %>% 
  select("sampleid","risklevel") %>%
  dplyr::arrange(desc(risklevel)) ## 排序样本，低风险在前
...
p_heatmap <- pheatmap(dat_heatmap,
                      ...
                      annotation_colors = list(Group = rlang::set_names(
                        c('#6AAFE6', '#ED9282'), c(con_name, treat_name))),
                      # 热图颜色
                      ...
                      main = "TCGA-STAD")
p_heatmap
ggsave(file = 'output/4-HeatMap.pdf', p_heatmap, width = 8, height = 6)

## 5.3 输出GSEA富集分析输入文件----
write.csv(res_final[,2,drop = F],"output/5-GSEA_input.csv")
